...
contract FlightSuretyApp {
    ...
    */
    ...
    modifier requireIsFundedAirline() {
        require(flightSuretyData.isFunded(msg.sender));
        _;
    }
    ...
    */   
                            ...
                            
    {
        ...
        bool success = flightSuretyData.registerAirline(newAddress, name);
     
        return success;
    }
    
    function fundAirline (address airline) external payable requireIsRegisteredAirline returns (uint256){
        address(flightSuretyData).transfer(msg.value); //send value to data contract
        uint256 amount = flightSuretyData.fund(airline, msg.value);
        return amount;
    }
   /**
...
interface FlightSuretyData {
    ...
    function registerAirline(address newAddress, string name) external returns (bool);
    function creditInsurees (string flight) external;
    function fund (address airline, uint256 amount) external returns(uint256);

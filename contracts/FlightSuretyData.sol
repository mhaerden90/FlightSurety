...
contract FlightSuretyData {
    ...
    struct Passenger {
        ...
        address wallet;
        uint256 credit;
        mapping(string => uint256) flightInsurances;        
    }
    mapping(string => address[]) private passengersInsuredForFlight;
    ...
    uint8 private constant max_insurance_amount = 1;
    ...
    */   
    function registerAirline
                            (address newAddress, string name 
                            )
                            ...
                            
    {
        ...
        airlineCount ++;
            ...
            for (uint c=0; c <airlines[newAddress].voted.length; c++){
                if (airlines[newAddress].voted[c] == msg.sender){
                    isDuplicate = true;
                    break;
                }
            }
            ...
            airlines[newAddress].voted.push(msg.sender);
    ...
    */   
    function buy
                            (string flightCode, string passengerName)                         
                            requireIsOperational
                            isAuthorizedCaller
                            external
                            payable
    {
            ...
            passengers[msg.sender].flightInsurances[flightCode] = msg.value;
            ...
            passengersInsuredForFlight[flightCode].push(msg.sender);
    ...
    */
    ...
    
    {
        ...
        for (uint256 c = 0; c < passengersInsuredForFlight[flightCode].length; c++) {
            address insuredPassengerAddress = passengersInsuredForFlight[flightCode][c];
            if(passengers[insuredPassengerAddress].flightInsurances[flightCode] != 0) {

                uint256 currentCredit = passengers[insuredPassengerAddress].credit;
                uint256 insuranceAmount = passengers[insuredPassengerAddress].flightInsurances[flightCode];
                passengers[insuredPassengerAddress].flightInsurances[flightCode] = 0; //reset to 0
